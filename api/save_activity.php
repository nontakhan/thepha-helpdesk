...
try {
    ...
    if ($action == 'create') {
        ...
        if (empty($activity_type_id) || empty($title) || empty($start_time_str)) { throw new Exception('กรุณากรอกข้อมูลประเภท, รายละเอียด และเวลาเริ่มต้นให้ครบถ้วน'); }
        $duration_minutes = null;
        if ($end_time_str) {
            $startTime = new DateTime($start_time_str); $endTime = new DateTime($end_time_str);
            if ($endTime < $startTime) { throw new Exception('เวลาสิ้นสุดต้องไม่น้อยกว่าเวลาเริ่มต้น'); }
        ...
        if ($stmt->execute()) { $response['success'] = true; $response['message'] = 'บันทึกกิจกรรมเรียบร้อยแล้ว'; } 
        else { throw new Exception('ไม่สามารถบันทึกข้อมูลได้: ' . $stmt->error); }
        $stmt->close();
    }
    // Action: สิ้นสุดกิจกรรมเก่า
    elseif ($action == 'finish') {
        $activity_id = (int)$_POST['activity_id']; $end_time_str = $_POST['end_time'];
        $stmt_get_start = $conn->prepare("SELECT start_time FROM activities WHERE id = ? AND admin_id = ?");
        $stmt_get_start->bind_param("ii", $activity_id, $admin_id); $stmt_get_start->execute();
        $result = $stmt_get_start->get_result();
        if ($result->num_rows === 0) { throw new Exception('ไม่พบกิจกรรมนี้ หรือคุณไม่มีสิทธิ์แก้ไข'); }
        $activity = $result->fetch_assoc(); $stmt_get_start->close();
        $startTime = new DateTime($activity['start_time']); $endTime = new DateTime($end_time_str);
        if ($endTime < $startTime) { throw new Exception('เวลาสิ้นสุดต้องไม่น้อยกว่าเวลาเริ่มต้น'); }
        $interval = $startTime->diff($endTime);
        $duration_minutes = ($interval->days * 24 * 60) + ($interval->h * 60) + $interval->i;
        ...
        if($stmt_update->execute()){ $response['success'] = true; $response['message'] = 'อัปเดตเวลาสิ้นสุดเรียบร้อยแล้ว'; } 
        else { throw new Exception('ไม่สามารถอัปเดตข้อมูลได้: ' . $stmt_update->error); }
        $stmt_update->close();
    }
    // Action: แก้ไขกิจกรรม
    elseif ($action == 'update') {
        ...
        if($stmt_update->execute()){
            if ($stmt_update->affected_rows > 0) { 
                ...
                $response['message'] = 'ข้อมูลไม่มีการเปลี่ยนแปลง'; 
            }
        } else { 
    ...
    elseif ($action == 'delete') {
        if (empty($_POST['activity_id'])) { throw new Exception('ไม่พบรหัสกิจกรรมที่ต้องการลบ'); }
        $activity_id = (int)$_POST['activity_id'];
        $stmt = $conn->prepare("DELETE FROM activities WHERE id = ?");
        $stmt->bind_param("i", $activity_id);
        if ($stmt->execute()) {
            if ($stmt->affected_rows > 0) { $response['success'] = true; $response['message'] = 'ลบกิจกรรมเรียบร้อยแล้ว'; } 
            else { throw new Exception('ไม่พบกิจกรรมที่ต้องการลบ (อาจถูกลบไปแล้ว)'); }
        } else { throw new Exception('ไม่สามารถลบข้อมูลได้: ' . $stmt->error); }
        $stmt->close();
    }
    else {
